<script lang="ts">
        ...
        case 'SELECT': {
          ...
          currentQuestion = {
            ...
            info: info?.tagName === 'P' || info?.tagName === 'FONT' ? info.textContent : undefined
					...
					console.log(`select ${currentQuestion.label}`);
					console.log(`info ${currentQuestion.info}`);
          currentSection.questions.push(currentQuestion);
          nameCount[baseName]++;
          break;
        }
        case 'INPUT': {
          const inputEl = ch as HTMLInputElement;
          const labelEl = ch.previousElementSibling?.tagName === 'FONT' ? ch.previousElementSibling : undefined;
          const infoEl = ch.nextElementSibling;
          const baseName = labelEl?.textContent ?? 'No name';
          const inputType = inputEl.type === 'text' || inputEl.type === 'hidden' || inputEl.type === 'submit' ? inputEl.type : undefined;
          if (inputType) {
            nameCount[baseName] ??= 0;
            currentQuestion = {
              name: `${baseName}${nameCount[baseName] ? `-${nameCount[baseName]}` : ''}`,
              type: 'input',
              inputType,
              label: labelEl?.textContent ?? 'No label',
              info: infoEl?.tagName === 'P' || infoEl?.tagName === 'FONT' ? infoEl.textContent : undefined,
							value: inputEl.value
            };
            currentSection.questions.push(currentQuestion);
            nameCount[baseName]++;
          } else {
            console.log(`skipped element ${inputEl.name} because it was type ${inputEl.type}`);
          }
          break;
        }
        default:
  ...
  $effect(() => {
    ...
    const form: DynForm = { action: formDiv.action, sections: [] };
  ...
  $effect(() => {
    window.parent.postMessage(`priceUpdated: ${fmdata.optionsTotal}`, '*');
    console.log(`Sent price update: ${fmdata.optionsTotal}`);
  });
</script>
